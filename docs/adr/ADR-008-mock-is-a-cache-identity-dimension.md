...
决定: **mock 进入缓存 identity 的 provider 子载荷**, 与 base_url / model / protocol 并列; 身份方案版本 1 -> 2. 配套三点: (1) TranslationJob 携带提交时的 provider 快照与命名空间, worker 用快照翻译而不是读活配置, 使「结果」与「写它用的身份」必然同源; (2) 命名空间 (provider 配置 + 系统提示词, 即 identity 把逐句字段留空后的值) 变动时, Engine 丢弃内存中属于旧 provider 的译文并重新调度当前句; (3) 晚到的旧命名空间结果在 _on_done 被丢弃.
